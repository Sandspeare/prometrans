...
def build_graph(function_dict: dict) -> nx.MultiDiGraph:
    ...
    for uuid in function_dict.keys():
        for callee_uuid in function_dict[uuid]["call_to"].keys():
            for call_site in function_dict[uuid]["call_to"][callee_uuid]:
...
def parse_json(engine_url: str, json_path: str, label: dict):
    ...
    for uuid in function_dict.keys():
        ...
        if project_name in label and function_info["function_name"] == "main":
            func_label = label[project_name]
        else:
            func_label = 0

        callee_graph = get_neighborhood_subgraph(call_graph, uuid, 2, 20, False)
        ...
        for file_name in tqdm(os.listdir(args.json_path)):
            if file_name.endswith(".json"):
                ...
                if args.num_workers == 1:
                    parse_json(engine_url, os.path.join(args.json_path, file_name), label)
                else:
                    pool.apply_async(parse_json, 
                                     args=(engine_url, 
                                           os.path.join(args.json_path, file_name), label))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--label_path", type=str, default=f"/path/to/POJdataset")
    args = parser.parse_args()

    for split in ["train", "test", "valid"]:
        print(split)

        args.db_path = f"/path/to/{split}data.db"
